...
import { createMetronomeEmbeddableLink, 
...
const MetronomeContext = createContext<MetronomeContextType>({
    ...
    fetchBalance: async () => {},
    fetchCosts: async() => { return {items:[], products:[]} },
    fetchAlerts: async () => { return {} },
    createAlert: async (threshold)=> { return {} },
...
export const MetronomeProvider: React.FC<{children: React.ReactNode}> = ({ children }) => {
    const [ metronome_config, setMetronomeConfig ] = useState<MetronomeConfig>({ api_key: "", customer_id:"", chart_type:"BarChart",});
    const [ balance, setBalance ] = useState<Balance>({ total_used: undefined, total_granted: undefined});
    const [ embeddable_url, setEmbedableUrl ] = useState<any>({ invoices: "", usage: "", credits:"", });
    const [ usage, setUsage ]  =  useState<Usage>({items: [], products: [],});
    const [ costs, setCosts ]  =  useState<Costs>({items: [], products: [], });
    const [ current_spend, setCurrentSpend ] = useState<number| undefined>(undefined);
    ...
    const fetchBalance = async():  Promise<void> => {
      try {
        const response = await fetchMetronomeCustomerBalance(metronome_config.api_key, metronome_config.customer_id)
    ...
    const fetchAlerts = async(): Promise<any> =>{
      try {
        return await fetchCustomerSpendAlerts( metronome_config.api_key, metronome_config.customer_id, );
      }
      catch (error) {
        return ;
      }
